...
import { Search, Calendar, Clock, ArrowRight, ArrowLeft, Newspaper, Sparkles, BookOpen, Flame, ExternalLink, Star, X } from 'lucide-react';
...
export default function GlassBlogPage({ posts, profile, settings, onNavigate, currentTheme, onThemeChange }) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col relative overflow-hidden transition-colors duration-300">
      ...
      <main className="flex-1 pt-28 pb-24 px-4 sm:px-6 max-w-6xl mx-auto w-full">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 space-y-3">
          <span className="inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-1.5 text-xs font-semibold text-cyan-600 dark:text-cyan-400 backdrop-blur-md shadow-sm">
            <Sparkles className="w-3.5 h-3.5" />
            Tech & AI Developer Publication
          </span>
        ...
        {/* Sleek Glassmorphism Search & Filter Bar */}
        <div className="mb-10 space-y-5">
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              ...
              className="w-full pl-11 pr-10 py-3 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl text-slate-900 dark:text-white text-xs sm:text-sm border border-slate-200 dark:border-white/15 shadow-sm focus:outline-none focus:border-cyan-500 dark:focus:border-cyan-400 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
            ...
            {STANDARD_CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer backdrop-blur-md ${
                    isActive
                      ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold shadow-md shadow-cyan-500/20 scale-[1.02]'
                      : 'bg-white/70 dark:bg-slate-900/60 text-slate-600 dark:text-slate-300 border border-slate-200/80 dark:border-white/10 hover:border-cyan-500/40 hover:text-slate-900 dark:hover:text-white shadow-xs'
                  }`}
                >
                  {cat}
                </button>
        ...
        {/* Featured Banner (Glass Style) */}
        {featuredPost && selectedCategory === 'All' && !search && (
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            onClick={() => onNavigate(`/blog/${featuredPost.slug}`)}
            className="mb-10 group cursor-pointer rounded-3xl bg-gradient-to-br from-cyan-950/80 via-slate-900/90 to-purple-950/80 p-6 sm:p-8 border border-cyan-500/30 text-white backdrop-blur-2xl shadow-xl relative overflow-hidden flex flex-col md:flex-row gap-6 items-center"
          >
            ...
            {featuredPost.cover_image && (
              <div className="w-full md:w-1/2 aspect-video rounded-2xl overflow-hidden bg-slate-950/50 flex-shrink-0 border border-white/10">
            ...
            <div className="w-full md:w-1/2 space-y-3 z-10">
              <div className="flex items-center gap-2">
                <span className="px-3.5 py-1 rounded-full bg-cyan-500 text-slate-950 font-extrabold text-[11px] uppercase tracking-wider flex items-center gap-1">
        ...
        {filteredPosts.length > 0 ? (
            ...
            {filteredPosts.map((post, idx) => {
              ...
              return (
                <motion.article
                  ...
                  className="group cursor-pointer rounded-3xl bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm hover:border-cyan-500/40 transition-all flex flex-col justify-between"
                >
                  <div>
                    <div className="relative aspect-video w-full overflow-hidden bg-slate-100 dark:bg-slate-950/40">
                      {post.cover_image ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center text-slate-400">
                          <Newspaper className="w-10 h-10 text-cyan-500/40" />
                        </div>
                      )}
                      
                      <div className="absolute top-3 left-3 flex items-center gap-1.5 flex-wrap">
                        <span className="px-3 py-1 rounded-full bg-white/90 dark:bg-slate-900/80 backdrop-blur-md text-cyan-600 dark:text-cyan-400 text-xs font-bold border border-slate-200 dark:border-white/10">
                          {post.category || 'Tech'}
                        </span>
                        {post.is_trending && (
                    ...
                    <div className="p-6">
                      ...
                      <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors line-clamp-2">
                        {post.title}
                      </h2>
          ...
          <div className="py-16 text-center bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-white/10 p-8 shadow-sm">
